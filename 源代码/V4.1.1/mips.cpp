...
void lw(char* reg_name,char*  vari_name)
{
    ...
    {
        //说明此时是索引的全局变量
        fprintf(fp_result,"\tla\t$s1,%s\n",vari_name);
        fprintf(fp_result,"\tlw\t%s,0($s1)\n",reg_name);
    }else
    {
...
void sw(char* reg_name,char*  vari_name)
{
    ...
    {
        //说明是一个全局变量
        fprintf(fp_result,"\tla\t$s2,%s\n",vari_name);
        fprintf(fp_result,"\tsw\t%s,0($s2)\n",reg_name);
    }else
    {
                ...
                for(int i=0;i<MaxBukSize;i++)
                {
                    ...
                    while(p)
                    {
                        if(p->type == TConst)
                        {
                            if(p->kind = Char)
                            {
                                fprintf(fp_result,"\tli\t$s5,%d\n",p->val.ch_val);
                            }else
                            {
                                fprintf(fp_result,"\tli\t$s5,%d\n",p->val.int_val);
                            }
                            fprintf(fp_result,"\tsw\t$s5,%d($fp)\n",find_addr(p->name));
                        }
                        p = p->next;
                    }
                }
