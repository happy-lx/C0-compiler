...
void typecheck(ASTNODE * tree)
{
    ...
    if(tree != NULL && tree->type != CONSTDECLARE && tree->type != VARIDECLARE)
    {
        ...
        }else if(tree->type == ASSIGN_STMT)
        {
            ...
            {
                //表示是可以的
                if(tree->childen[0]->type == IDTYPE)
                {
                    p = find_table(tree->childen[0]->attr.name);
                    if(p->type == TConst)
                    {
                        //发生常量赋值错误
                        printf("Sementic Error at line %d : assign to variable of const declared\n",tree->lineno);
                        exit(1);
                    }
                }
        ...
        }else if(tree->type == SCANF_STMT)//5月24日添加，scanf不可以对常量进行赋值
        {
            t = tree->childen[0];

            while(t)
            {
                p = find_table(t->attr.name);
                if(p->type == TConst)
                {
                    printf("Semantic Error at line %d : assign to variable of const declared\n",tree->lineno);
                    exit(1);
                }
                t = t->sibling;
            }
