...
void match(tokentype input)
{
    ...
    {
        //错误处理，不跳转
        sprintf(emsg,"Error at line %d : \n %s is not expected , expect token id is %d\n",linenumber,tokenvalue,input);
        error_msg(emsg,no_skip);
    }
}
...
ASTNODE* program()
{
    ...
    while(token == INT || token == CHAR)
    {
        ...
        if(token == ID)
        {
            ...
            {
                //错误处理，调到void处
                sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as ( , DOT , ; , [\n",linenumber,tokenvalue);
                error_msg(emsg,skip_to_void);
                break;
            }
        }else
        {
            //错误处理，调到void处
            sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as ID\n",linenumber,tokenvalue);
            error_msg(emsg,skip_to_void);
            break;
        }
    }
    ...
    while(token == INT || token == CHAR || token == VOID || (token == LB && pretoken == ID && prepretoken != VOID))
    {
        ...
        }else if(token == VOID)
        {
            ...
            }else if(token == ID)
            {
                ...
                copypre();
                match(token);
                result->childen[2] = void_fun_dec();//preprevalue为VOID，pretokenvalue是id，token现在是LBf
                break;//6月2日添加  去识别main
            }else
            {
                //错误处理，调到void处
                sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as ID , MAIN\n",linenumber,tokenvalue);
                error_msg(emsg,skip_to_void);
                break;
            }
        }
    ...
    {
        //错误处理，调到endfile处
        sprintf(emsg,"Error at line %d : \n %s is not expected , You have to declare a main function\n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_endfile);
    }
...
ASTNODE* const_dec()//需要重构
{
    ...
    {
        //错误处理，调到void处
        sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as int , char\n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_void_int_char);
        return NULL;
    }
    match(token);
    if(token == ID)
    {
        ...
        {
            //错误处理，调到void处
            sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as integer , character\n",linenumber,tokenvalue);
            error_msg(emsg,skip_to_void_int_char);
            return NULL;
        }
        p->childen[1] = temp2;
...
ASTNODE* vari_dec()//需要重构
{
    ...
    if(token == SEMI)
    {
        ...
        {
            //错误处理，调到void处
            sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as int , char , void , all kinds of statements \n",linenumber,tokenvalue);
            error_msg(emsg,skip_to_void);
            return NULL;
        }
    }
    ...
    if(token == DOT || token == LSB)//int a , c[2] , d , e , f;
    {
        ...
        else
        {
            ...
            {
                //错误处理，调到void处
                sprintf(emsg,"Error at line %d : \n %s is not expected , expect token as integer \n",linenumber,tokenvalue);
                error_msg(emsg,skip_to_void);
                return NULL;
            }
...
ASTNODE* no_void_fun_dec()
{
    ...
    {
        //错误处理，调到void处
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as void int char \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_void);
        return NULL;
    }
}
...
ASTNODE* arg_lists()
{
    ...
    {
        //错误处理，调到RB的下一个处
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as char , int , ) \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RB_next);
        return NULL;
    }
}
ASTNODE* arg_list()
{
    ...
    {
        //错误处理，调到RB的下一个处
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as id \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RB_next);
        return NULL;
    }
    ...
    {
        //错误处理，调到RB的下一个处
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as DOT , ) \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RB_next);
        return NULL;
    }
        return result;
...
ASTNODE* compond_stmt()
{
    ...
    default:
        switch(token)
        {
            ...
            default :
                    //错误处理，调到RBB
                    sprintf(emsg,"Error at line %d : \n %s is not expected , expected all kinds of statements \n",linenumber,tokenvalue);
                    error_msg(emsg,skip_to_RBB);
                    return NULL;
        }
    }
    return result;
...
ASTNODE* stmt_seq()
{
    ...
    {
        //错误处理，调到RBB
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected all kinds of statements or } \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RBB);
        return NULL;
    }
}
ASTNODE* stmt()
{
    ...
    case ID:
        ...
        {
            //06/02日添加，既不是赋值语句又不是函数调用语句则跳到;下一个
            sprintf(emsg,"Error at line %d : \n %s is not expected , expected all kinds of statements \n",linenumber,tokenvalue);
            error_msg(emsg,skip_to_SEMI_next);
            return NULL;
        }
        break;
    default:
...
ASTNODE* for_stmt()
{
    ...
    }else
    {
        //错误处理，调到}的下一个
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as id \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RBB_next);
        return NULL;
    }
    ...
    {
        //错误处理，调到}的下一个
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as + , - \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RBB_next);
        return NULL;
    }
    ...
    {
        //错误处理，调到}的下一个
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as integer \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RBB_next);
        return NULL;
    }
...
ASTNODE* printf_stmt()
{
    ...
    {
        //错误处理，调到;
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as + , - , id , integer , character , )  \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_SEMI);
        return NULL;
    }
...
ASTNODE* factor()//在构建符号表时，可能对常量还需要处理一下
{
    ...
    switch(token)
    {
        ...
        case LB:
            ...
            return result;
        default :
            //错误处理，跳过这个单词，忽略它06/02日修改
            sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as integer , charactor , id , (   \n",linenumber,tokenvalue);
            error_msg(emsg,skip_to_next);
            return NULL;
...
ASTNODE* arg_values()
{
    ...
    if(token == PLUS || token == MINUS || token == ID || token == INTGE || token == CHR || token == LB)
    {
        ...
        return p;
    }else if(token == RB)//5月24日修改，不知是否会不妥
    {
        return NULL;
    }else
    {
        //错误处理，调到)
        sprintf(emsg,"Error at line %d : \n %s is not expected , expected token as + , - , id , integer , charactor , ( , )   \n",linenumber,tokenvalue);
        error_msg(emsg,skip_to_RB);
        return NULL;
